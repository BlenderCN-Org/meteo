...
class DynamicExplosion:
    ...
    def display_loop(self):
        d = DynamicDisplay()
        d.dynamic_display(self.img)
        print("Je ne sort pas de là !!!!")
        print("Seulement avec Echap !!!!")

    def display_loop_start(self):
        thread_d = threading.Thread(target=self.display_loop)  #, args=(self.img,))
        thread_d.start()
    ...
    def explosion(self):
        ...
        global IMG
        #self.display_loop_start()

        while self.loop:
            #print("Nouvelle image explosion")
            img = get_black_image()
            IMG = self.get_circles_at_t(img)
            sleep(0.03)

    def explosion_simple(self):
        '''Affichage de l'explosion.'''
...
def DynamicExplosion_test():
    ...
    # Création d'une explosion
    de = DynamicExplosion(O, n, start, gap, ampli, pixel, slide, decrease)
    de.explosion()
...
def test():
    ...
    print("DynamicExplosion_simple_test")
    DynamicExplosion_simple_test()

    print("DynamicExplosion_test")
...
def HalfWave_test():
    ...
    # Création de la demi sinus
    hw = HalfWave(O, start, gap, ampli, sens=0)

    # Récup image avec demi sinus
    img = hw.draw_half_wave(get_black_image())

    # Affichage fixe
    StaticDisplay().static_display(img)
...
def Wave_test():
    ...
    # Création des n demi sinus
    w = Wave(n, O, start, gap, ampli)

    # Récup image avec les n sinus
    img = get_black_image()
...
def StaticExplosion_test():
    ...
    # Création des cercles des n demi sinus
    se = StaticExplosion(O, n, start, gap, ampli, pixel)

    # Récup image avec les n sinus
    img = get_black_image()
    img = se.lines_to_circle(img)

    # Affichage fixe
    StaticDisplay().static_display(img)

def DynamicExplosion_simple_test():
    n = 8
    O = 200, 400
    start = 20
    gap = 30
