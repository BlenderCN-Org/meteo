...
def superposition(background, foreground, decal):
    ...
    rows_fg, cols_fg, ch = foreground.shape
    ...
    # Put fg on bg
    background[b:h, a:l] = cv2.add(bg, fg)
...
def display_image():
    ...
    wave_ori = load_image_RVB(IMG)

    # Scale initial
    wave = scale_image(wave_ori.copy(), SCALE[0])
    wave2 = scale_image(wave_ori.copy(), SCALE[0])

    C = (randint(0, 1200), randint(0, 800))
    C2 = (randint(0, 1200), randint(0, 800))
    t_init = time()
    ...
    while "Pas d'appui sur Echap":
        ...
        wave2 = scale_image(wave, SCALE[1])

        a, b = center_to_origin(wave, C)
        foreground, decal = selection_position(wave, a, b)
        a2, b2 = center_to_origin(wave2, C2)
        foreground2, decal2 = selection_position(wave2, a2, b2)

        if decal:
            # Image finale
            img = superposition(bg_img, foreground, decal)
            img2 = superposition(img, foreground2, decal2)
